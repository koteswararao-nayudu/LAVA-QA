...
chan.setblocking(0)
chan.exec_command(' /usr/bin/python3 /home/equser/koti/lava-testcases/cronjobs/GP_Build_trigger_cronjob.py >> /home/equser/koti/lava-testcases/cronjobs/GP_logs_RC5.1.txt' )
...
_stdin, _stdout1, _stderr = client.exec_command("lavacli --uri http://admin:es3bkjwhjo04oazzl0zqhl7zfr2n8wf1eg3zl246o9gspyyi2pczdsotmufgyc00yzkjuz6o0pl28mymjz7he980artaysb7hj4h4r5tia1u2f3y3ecf94ys1ye32ns0@192.168.3.198/RPC2/ jobs submit /home/equser/koti/lava-testcases/e2e/board46/tmp/edgeq-raptor2_e2e_gnb_cellup_board46-generic1_RC5.1.yaml")
...
windows_results_stress_file_path = "//192.168.3.230/public/lava-qa/results/windows-results/cell_attach_stress.txt"
...
if os.path.exists(windows_results_stress_file_path):
    os.remove(windows_results_stress_file_path)
else:
    print("Does not find the file")
...
rcv21 = port.read(80)
rcv21_strip=rcv21.strip().decode( "utf-8" )
print(repr(rcv21_strip))
...
rcv22 = port.read(80)
rcv22_strip=rcv22.strip().decode( "utf-8" )
print(repr(rcv22_strip))
...
if os.path.exists("C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file.txt"):
    print("Removing netsh_show_interfaces_file.txt")
    os.remove("C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file.txt")
    
netshcmd1 = subprocess.Popen("netsh interface ipv4 show interfaces >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file.txt", stdout=subprocess.PIPE, shell=True)
netshcmd_status1 = netshcmd1.wait()
output_netshcmd1, errors =  netshcmd1.communicate()
print(output_netshcmd1)
print("Command exit status/return code : ", netshcmd_status1)

cellular_id_name = subprocess.Popen("grep \"Cellular\"  C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file.txt  | awk \"{$1=$2=$3=\\\"\\\"; print $0}\" | awk \"{$1=\\\"\\\";print $0}\" ", stdout=subprocess.PIPE, shell=True)
...
celluar_grep_command='netsh interface ipv4 show addresses \"' + output_cellular_id_name_strip + '\" >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file.txt'
netshcmd = subprocess.Popen( celluar_grep_command , stdout=subprocess.PIPE, shell=True)
...
if output_telit_ipaddress_strip == nullstring :
    ...
    time.sleep(2)
    myScreenshot.save(os.path.join(cwd1,"1_" + strip_str_lava_job_id + "_ipconfig_FAIL.jpg"))
    time.sleep(2)
    ...
    file_path_stress = "//192.168.3.230/public/lava-qa/results/windows-results/cell_attach_stress.txt"
    
    #stress
    for x in range(102) :
            port.write(b"at \r\n")
            rcv3stress = port.read(80)
            rcv3stress_strip=rcv3stress.strip().decode( "utf-8" )
            print(repr(rcv3stress_strip))
            port.write(b"at+cfun=0 \r\n")
            rcv4stress = port.read(80)
            rcv4stress_strip=rcv4stress.strip().decode( "utf-8" )
            print(repr(rcv4stress_strip))
            time.sleep(15)
            port.write(b"at+cfun=1 \r\n")
            time.sleep(1)
            rcv5stress = port.read(82)
            rcv5stress_strip=rcv5stress.strip().decode( "utf-8" )
            print(repr(rcv5stress_strip))

            rcv21stress = port.read(80)
            rcv21stress_strip=rcv21stress.strip().decode( "utf-8" )
            print(repr(rcv21stress_strip))
            port.write(b"at+cpin? \r\n")
            rcv21stress = port.read(80)
            rcv21stress_strip=rcv21stress.strip().decode( "utf-8" )
            print(repr(rcv21stress_strip))

            rcv22stress = port.read(80)
            rcv22stress_strip=rcv22stress.strip().decode( "utf-8" )
            print(repr(rcv22stress_strip))
            port.write(b"at+cimi \r\n")
            rcv22stress = port.read(80)
            rcv22stress_strip=rcv22stress.strip().decode( "utf-8" )
            print(repr(rcv22stress_strip))
            ...
            if os.path.exists("C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file_stress.txt"):
                print("Removing netsh_show_interfaces_file_stress.txt")
                os.remove("C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file_stress.txt")
                
            netshcmd1_stress = subprocess.Popen("netsh interface ipv4 show interfaces >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file_stress.txt", stdout=subprocess.PIPE, shell=True)
            netshcmd_status1_stress = netshcmd1_stress.wait()
            output_netshcmd1_stress, errors =  netshcmd1_stress.communicate()
            print(output_netshcmd1_stress)
            print("Command exit status/return code : ", netshcmd_status1_stress)

            cellular_id_name_stress = subprocess.Popen("grep \"Cellular\"  C:/Users/DELL/Desktop/koti/koti/tmp/telit/netsh_show_interfaces_file_stress.txt  | awk \"{$1=$2=$3=\\\"\\\"; print $0}\" | awk \"{$1=\\\"\\\";print $0}\" ", stdout=subprocess.PIPE, shell=True)
            cellular_id_name_status_stress = cellular_id_name_stress.wait()
            output_cellular_id_name_stress, errors =  cellular_id_name_stress.communicate()
            print(output_cellular_id_name_stress)
            output_cellular_id_name_strip_stress = output_cellular_id_name_stress.strip().decode( "utf-8" )
            print(output_cellular_id_name_strip_stress)
            print("Command exit status/return code : ", cellular_id_name_status_stress)


            if os.path.exists("C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file_stress.txt"):
                print("Removing ipaddress_file.txt")
                os.remove("C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file_stress.txt")


            celluar_grep_command_stress='netsh interface ipv4 show addresses \"' + output_cellular_id_name_strip + '\" >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file_stress.txt'
            netshcmd_stress = subprocess.Popen( celluar_grep_command_stress , stdout=subprocess.PIPE, shell=True)
            #netshcmd = subprocess.Popen("netsh interface ipv4 show addresses \"Cellular\" >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file.txt ", stdout=subprocess.PIPE, shell=True)
            netshcmd_status_stress = netshcmd_stress.wait()
            output_netshcmd_stress, errors =  netshcmd_stress.communicate()
            print(output_netshcmd_stress)
            print("Command exit status/return code : ", netshcmd_status_stress)
            #ipp = subprocess.Popen(netsh show >> C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file.txt )

            #if os.path.exists("C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file.txt"):
            #    os.remove("C:/Users/DELL/Desktop/koti/koti/tmp/telit/serial_ports.txt")

            telit_ipaddress_stress = subprocess.Popen("grep \"IP Address\" C:/Users/DELL/Desktop/koti/koti/tmp/telit/ipaddress_file_stress.txt | awk \"{ print $3 }\"", stdout=subprocess.PIPE, shell=True)
            telit_ipaddress_status_stress = telit_ipaddress_stress.wait()
            output_telit_ipaddress_stress, errors =  telit_ipaddress_stress.communicate()
            print(output_telit_ipaddress_stress)
            output_telit_ipaddress_strip_stress = output_telit_ipaddress_stress.strip().decode( "utf-8" )
            print(output_telit_ipaddress_strip_stress)
            print("Command exit status/return code : ", telit_ipaddress_status_stress)
            
            if output_telit_ipaddress_strip_stress == nullstring :
                print("test fail")
                print("Could not locate the IP Address")
                pag.press("winleft", _pause=True)
                time.sleep(1)
                ...
                #pag.press("enter")
                #time.sleep(2)
                print("Taking screenshot for ipconfig FAIL")
                myScreenshot = pyautogui.screenshot()
                time.sleep(2)
                myScreenshot.save(os.path.join(cwd1,"stress_" + x + "_" + strip_str_lava_job_id + "_ipconfig_FAIL.jpg"))
                time.sleep(2)
                
                pag.typewrite("exit",interval=0.2)
                pag.press("enter")
                time.sleep(3)
                
                attach_file_stress = open(file_path_stress,'w')
                attach_file_stress.write("fail")
                attach_file_stress.close()
                exit()
            else:
                print("test pass")
                #pag.press("winleft", _pause=True)
                ...
                time.sleep(2)
                xstr=str(x)
                myScreenshot.save(os.path.join(cwd1,"stress_" + xstr + "_" + strip_str_lava_job_id + "_ipconfig_PASS.jpg"))
                time.sleep(2)
